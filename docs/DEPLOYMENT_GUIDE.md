...
3. Set it to: `/dashboard`
   - This tells Railway to treat `dashboard/` as the project root
...
With Root Directory set to `/dashboard`, Railway will:
...
1. Go to **Variables** tab
2. Add frontend env vars from `dashboard/.env.example`:
...
1. Make a change to any file in `dashboard/`
2. Push to GitHub
...
|---------|-----------------|------------------|
| **Root Directory** | `/` (empty) | `/dashboard` |
| **Watch Paths** | `backend/**`, `start.sh`, `railway.json`, `nixpacks.toml` | `dashboard/**` |
...
- Ensure Root Directory is exactly `/dashboard`
...
| `.dockerignore` | Excludes `dashboard/` from backend builds |
| `dashboard/.dockerignore` | Excludes `backend/` from frontend builds |
