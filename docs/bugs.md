...
- 2026-06-18 | `scripts/pinescript/indicators/SND_Raw_RD_Forex_LAB.pine`, `scripts/pinescript/indicators/SND_Raw_RD_Forex_PROD.pine` | One-candle/local demand liquidity could draw the inducement line at the wick extreme instead of the body-side RD level, moving the 23:10 orange-zone line below the expected level | For local one-candle liquidity only, link/display the body-side low/high while keeping strict pivot liquidity wick-based
