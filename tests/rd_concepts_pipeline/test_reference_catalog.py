...
def test_first_manual_case_remains_provisional_and_out_of_release_coverage() -> None:
    records = load_rule_catalog(REFERENCE / "rd_5m_rules.jsonl")
    cases = load_benchmark_cases(REFERENCE / "rd_5m_cases.jsonl")

    assert len(cases) == 1
    assert cases[0]["case_id"] == "USDJPY-5M-FORMATION-WICK-001"
    assert cases[0]["label_status"] == "PROVISIONAL"
    ...
    assert "RD5M-ZONE-FORMATION-WICK-DEMAND" in coverage["missing_positive"]
