...
def test_get_ai_runs_bulk_falls_back_to_signal_score_for_stuck_pending_run(monkeypatch) -> None:
    class _Resp:
        def __init__(self, data):
            self.data = data
    ...
    class _Supabase:
        ...
        def table(self, name: str):
            ...
            if name == "trading_signals":
                return _Query([{"id": 12, "score": 87, "ai_confidence": 55.5}])
            return _Query([])
    ...
    assert run["status"] == "complete"
    assert run["recommendation"] == "allow"
    assert run["confidence"] == 87
    assert run["votes"] == {}
    assert run["source"] == "signal_score"
