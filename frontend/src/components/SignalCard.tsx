...
function ConfidenceBar({ score }: { score: number | null }) {
    ...
    return (
      <div className='w-full space-y-1'>
        <div className='flex items-center justify-between'>
          <span className='text-[10px] font-mono text-[var(--to-text-dim)] uppercase tracking-wider flex items-center gap-1'>
            <Sparkles className='w-3 h-3' />
            AI Confidence
          </span>
          <span className='font-mono text-xs text-[var(--to-text-dim)]'>N/A</span>
        </div>
        <div className='h-2 w-full bg-[var(--to-surface-raised)]/50 rounded-full overflow-hidden'>
          <div className='h-full w-0 bg-[var(--to-surface-raised)] rounded-full' />
        </div>
      </div>
    );
  }
  ...
  const getTextColor = (s: number) => {
    if (s >= 80) return 'text-[var(--to-long)]';
...
export function SignalCard({ signal, onInspect }: SignalCardProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'group relative bg-[var(--to-surface)]/70 border rounded-lg overflow-hidden',
        'transition-all duration-200 cursor-pointer select-none',
        'hover:bg-[var(--to-surface)]/90 hover:border-[var(--to-border)]',
    ...
    >
      ...
      <div className='flex items-center justify-between p-3 border-b border-zinc-800/60'>
        <div className='flex items-center gap-2.5'>
          <span className='font-mono text-base font-bold text-[var(--to-text-primary)] tracking-tight'>
            {symbol}
          </span>
          <SideBadge side={side} />
        </div>
      ...
      <div className='px-3 pt-3 pb-2 space-y-1.5'>
        ...
        {signal.broker_order_id && (
          <div className='flex items-center justify-between'>
            <span className='text-[10px] font-mono text-[var(--to-text-dim)] uppercase'>
              Ticket
            </span>
      ...
      <div className='px-3 pb-2 flex items-center justify-between gap-4'>
        ...
        <div className='flex items-center gap-2'>
          {(signal.price ?? signal.entry) ? (
            ...
            <span className='text-[10px] font-mono text-[var(--to-text-dim)]'>--</span>
          )}
        </div>
      ...
      <div className='px-3 pb-3 space-y-2'>
        <div className='min-h-[32px]'>
          {reason ? (
            <p className='text-[11px] font-mono text-[var(--to-text-dim)] leading-relaxed line-clamp-2'>
              {reason}
            </p>
          ) : (
            <p className='text-[11px] font-mono text-[var(--to-text-dim)] italic'>
...
export function SignalCardSkeleton() {
  return (
    <div className='bg-[var(--to-surface)]/70 border border-zinc-800 rounded-lg overflow-hidden animate-pulse'>
      <div className='flex items-center justify-between p-3 border-b border-zinc-800/60'>
        <div className='flex items-center gap-2.5'>
          <div className='h-5 w-20 bg-[var(--to-surface-raised)] rounded' />
          <div className='h-5 w-14 bg-[var(--to-surface-raised)] rounded-full' />
        </div>
        <div className='h-5 w-16 bg-[var(--to-surface-raised)] rounded-full' />
      </div>
      <div className='px-3 pt-3 pb-2 space-y-1.5'>
        <div className='flex items-center justify-between'>
          <div className='h-3 w-24 bg-[var(--to-surface-raised)] rounded' />
          <div className='h-3 w-10 bg-[var(--to-surface-raised)] rounded' />
        </div>
        <div className='h-2 w-full bg-[var(--to-surface-raised)] rounded-full' />
      </div>
      <div className='px-3 pb-2 flex items-center justify-between gap-4'>
        <div className='h-3 w-20 bg-[var(--to-surface-raised)] rounded' />
        <div className='h-3 w-14 bg-[var(--to-surface-raised)] rounded' />
        <div className='h-3 w-16 bg-[var(--to-surface-raised)] rounded' />
      </div>
      <div className='px-3 pb-3 space-y-2'>
        <div className='space-y-1'>
          <div className='h-3 w-full bg-[var(--to-surface-raised)] rounded' />
          <div className='h-3 w-3/4 bg-[var(--to-surface-raised)] rounded' />
        </div>
        <div className='flex items-center justify-between pt-2 border-t border-zinc-800/40'>
          <div className='h-2.5 w-16 bg-[var(--to-surface-raised)] rounded' />
          <div className='h-2.5 w-14 bg-[var(--to-surface-raised)] rounded' />
        </div>
      </div>
    </div>
  );
