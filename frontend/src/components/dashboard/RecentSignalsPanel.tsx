...
function StatusBadge({ status, pnl }: { status: string; pnl?: number | null }) {
  ...
  const configs: Record<string, { icon: React.ReactNode; label: string; className: string }> = {
    active: {
      ...
      label: 'LIVE',
      className: 'bg-[#2962ff]/15 text-[#2962ff] border-[#2962ff]/30',
    },
    closed: {
    ...
    ai_rejected: {
      ...
      label: 'VETO',
      className: 'bg-[#ef5350]/15 text-[#ef5350] border-[#ef5350]/30',
    },
    filtered: {
      icon: <FilterIcon className="w-2.5 h-2.5" />,
      label: 'FILTER',
    ...
    pending: {
      ...
      label: 'PENDING',
      className: 'bg-[#ff9800]/15 text-[#ff9800] border-[#ff9800]/30',
    },
  };
  ...
  return (
    <Badge className={cn('font-mono text-[9px] font-semibold gap-1 border px-1.5 py-0', config.className)}>
      {config.icon}
      {config.label}
    </Badge>
  );
...
export function RecentSignalsPanel({ mode, onSelectSignal }: RecentSignalsPanelProps) {
      ...
      <ScrollArea className="flex-1">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <span className="text-xs text-zinc-600 font-mono">No signals match filter</span>
          </div>
        ) : (
          <Table className="table-dense">
            <TableHeader>
              <TableRow className="border-b border-[#2a2e39] hover:bg-transparent">
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[80px]">Time</TableHead>
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[120px]">Signal</TableHead>
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[100px]">Status</TableHead>
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[50px]">AI</TableHead>
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[55px] text-right">R:R</TableHead>
                <TableHead className="font-mono text-[9px] text-zinc-600 uppercase tracking-wider py-1.5 px-3 w-[70px] text-right">PnL</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.map((signal) => {
                  ...
                  >
                    <TableCell className="py-2 px-3">
                      <span className="font-mono text-[10px] text-zinc-600 tabular-nums">
                        {formatDistanceToNowStrict(new Date(signal.created_at), { addSuffix: true })}
                      </span>
                    </TableCell>
                    <TableCell className="py-2 px-3">
                    ...
                    <TableCell className="py-2 px-3 text-right">
                      {pnl != null ? (
                        <span className={cn('font-mono text-[11px] font-bold tabular-nums', pnl >= 0 ? 'text-[#26a69a]' : 'text-[#ef5350]')}>
                          {pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}
                        </span>
                      ) : (
