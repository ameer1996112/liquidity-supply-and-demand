  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-[#2a2e39] bg-[#1e222d]/80 p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] text-zinc-500 font-mono block mb-1">Account type</label>
          <select
            value={accountType}
            onChange={(e) => {
        ...
        <div>
          <label className="text-[10px] text-zinc-500 font-mono block mb-1">Provider</label>
          <select
            value={provider}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-[10px] text-zinc-500 font-mono block mb-1">Starting capital ($)</label>
          <input
            type="number"
            min={0}
            step={100}
      ...
      {/* MetaAPI Connection */}
      <div>
        <label className="text-[10px] text-zinc-500 font-mono block mb-1">MetaAPI Account ID</label>
        <input
          type="text"
      ...
      {showAdvanced && (
        <div className="space-y-3 pl-2 border-l border-[#2a2e39]">
          <div>
            <label className="text-[10px] text-zinc-500 font-mono block mb-1">Token env key</label>
            <input
              type="text"
            ...
            />
            <p className="text-[9px] text-zinc-600 mt-1">
              Environment variable name holding the MetaAPI JWT. Default: META_API_TOKEN
            </p>
          </div>
        </div>
      )}
      ...
      {accountType === 'Eval' && (
        <div className="rounded border border-blue-500/30 bg-blue-500/5 px-3 py-2">
          <p className="text-[10px] text-blue-400">
            Evaluation mode will be enabled. The bot will enforce daily loss (5%) and max drawdown (10%) limits per FTMO rules.
            You can customize limits in the account settings after creation.
          </p>
        </div>
      )}
