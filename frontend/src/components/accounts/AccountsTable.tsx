...
export function AccountsTable({ accounts, onDelete }: AccountsTableProps) {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'balance' | 'daily_pnl' | 'total_pnl'>('name');
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#2a2e39] bg-[#1e222d]/50">
              ...
              <th className="text-left px-4 py-3 text-xs font-mono text-zinc-500">Type</th>
              <th className="text-left px-4 py-3 text-xs font-mono text-zinc-500">Connection</th>
              ...
              <th className="text-right px-4 py-3 text-xs font-mono text-zinc-500">Equity</th>
              <th className="text-right px-4 py-3 text-xs font-mono text-zinc-500">
                <button
                  onClick={() => {
                    setSortBy('daily_pnl');
              ...
              <th className="text-center px-4 py-3 text-xs font-mono text-zinc-500">Positions</th>
              <th className="text-right px-4 py-3 text-xs font-mono text-zinc-500">Win Rate</th>
            ...
            {sortedAccounts.map((account) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {account.account_type ? (
                      <span className={cn(
                        ...
                        account.account_type === 'Eval' && 'bg-amber-500/10 text-amber-400 border border-amber-500/20',
                        account.account_type === 'Funded' && 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20',
                        account.account_type === 'Personal' && 'bg-zinc-700/30 text-zinc-400'
                      )}>
                        {account.account_type}
                      </span>
                    ) : (
                  ...
                  {/* Win Rate */}
                  <td className="px-4 py-3 text-right">
                    <span className={cn('font-mono text-xs font-medium', winRateClass)}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        onClick={() => syncMutation.mutate(account.account_name)}
