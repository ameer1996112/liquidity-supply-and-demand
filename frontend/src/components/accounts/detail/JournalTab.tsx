...
async function fetchJournalEntries(accountName: string, tagFilter?: string): Promise<JournalEntry[]> {
  const url = getPortfolioControlUrl(`/accounts/${encodeURIComponent(accountName)}/journal${tagFilter ? `?tag=${encodeURIComponent(tagFilter)}` : ''}`);
...
async function createJournalEntry(accountName: string, entry: {
  signal_id?: number;
  note_text: string;
  tags?: string[];
  trade_rating?: number;
  ...
  const url = getPortfolioControlUrl(`/accounts/${encodeURIComponent(accountName)}/journal`);
  if (!url) throw new Error('Backend API URL not configured');
...
export function JournalTab({ accountName }: JournalTabProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (entry: Parameters<typeof createJournalEntry>[1]) => createJournalEntry(accountName, entry),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['account-journal', accountName] });
    ...
    <div className="space-y-6">
      ...
      {showAddForm && (
        <div className="tv-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-zinc-500 mb-2">Rating (1-5)</label>
                ...
                {[1, 2, 3, 4, 5].map(n => (
                  <button
                    ...
                    className={cn(
                      ...
                      rating && n <= rating ? 'text-amber-500' : 'text-zinc-600 hover:text-amber-500'
                    )}
                  >
          ...
          <div>
            <label className="block text-xs text-zinc-500 mb-2">Emotional State (Optional)</label>
            <select
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              !tagFilter ? 'bg-emerald-500/20 text-emerald-400' : 'text-zinc-500 hover:text-zinc-300'
            )}
          >
            All
          </button>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-600">
          Journal endpoints not yet implemented. This will show your trade notes, tags, and ratings.
        </div>
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-[#2a2e39] bg-[#1e222d]/50 py-16 text-zinc-500">
          ...
          <p className="text-xs text-zinc-600 mt-1 max-w-md text-center">
            Click "Add Note" to start documenting your trading journey.
            Track what works, what doesn't, and your emotional state.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {entry.trade_outcome && entry.trade_outcome !== 'open' && (
                      <span className={cn(
                        ...
                        {entry.trade_pnl && ` ${entry.trade_pnl >= 0 ? '+' : ''}$${entry.trade_pnl.toFixed(2)}`}
                      </span>
                    )}
                    {entry.trade_rating && (
                      <div className="flex items-center gap-0.5">
                        {Array.from({ length: entry.trade_rating }).map((_, i) => (
                  ...
                  {entry.emotional_state && (
                    <div className="mt-2 text-xs text-zinc-500">
                      <span className="text-zinc-600">Mood:</span> {entry.emotional_state}
                    </div>
                  )}
