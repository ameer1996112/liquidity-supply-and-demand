...
async function fetchPairStats(accountName: string): Promise<PairStats[]> {
  const url = getPortfolioControlUrl(`/accounts/${encodeURIComponent(accountName)}/analytics/pairs`);
...
async function fetchLosingStreaks(accountName: string): Promise<LosingStreak[]> {
  const url = getPortfolioControlUrl(`/accounts/${encodeURIComponent(accountName)}/analytics/streaks`);
  if (!url) throw new Error('Backend API URL not configured');
  const res = await fetch(url, { signal: AbortSignal.timeout(15000) });
  if (!res.ok) throw new Error(`Failed to fetch losing streaks (${res.status})`);
  const data = await res.json();
  return data.streaks || [];
}

export function AnalyticsTab({ accountName }: AnalyticsTabProps) {
  const { data: pairStats, isLoading: loadingPairs, error: pairError } = useQuery({
    queryKey: ['account-analytics-pairs', accountName],
    queryFn: () => fetchPairStats(accountName),
    staleTime: 60_000,
  });

  const { data: streaks, isLoading: loadingStreaks, error: streaksError } = useQuery({
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {!loadingPairs && pairStats && (
            <span className="text-xs text-zinc-500">{pairStats.length} pairs traded</span>
          )}
        </div>
        ...
        {loadingPairs ? (
          ...
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-600">
            Analytics endpoints not yet implemented. This will show per-pair win rate, profit factor, and avg hold time.
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#2a2e39] bg-[#1e222d]/50">
                    <th className="text-left px-4 py-2 text-xs font-mono text-zinc-500">Pair</th>
                    <th className="text-center px-4 py-2 text-xs font-mono text-zinc-500">Trades</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">Win Rate</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">Total P&L</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">Avg Win</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">Avg Loss</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">PF</th>
                    <th className="text-right px-4 py-2 text-xs font-mono text-zinc-500">Avg Hold</th>
                  </tr>
                </thead>
                <tbody>
                  {pairStats.map((pair) => {
                    ...
                    return (
                      <tr key={pair.symbol} className="border-b border-[#2a2e39] hover:bg-[#1e222d]/30">
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={cn(
                            ...
                            {isProfitable ? '+' : ''}${pair.total_pnl.toFixed(2)}
                          </span>
                        </td>
      ...
      <section className="space-y-3">
          ...
          {!loadingStreaks && streaks && streaks.length > 0 && (
            <span className="text-xs text-zinc-500">
              {streaks.filter(s => s.status === 'active').length > 0 && (
                <span className="text-amber-500">⚠️ Active streak detected</span>
              )}
            </span>
          )}
        </div>
        ...
        {loadingStreaks ? (
          ...
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-600">
            Streak detection endpoint not yet implemented. This will show consecutive losing trades with total loss and affected pairs.
          </div>
          ...
          <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-600">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">No significant losing streaks detected</span>
            </div>
            <p className="text-xs mt-1 text-emerald-600/80">
              You're managing risk well. Streaks of 3+ consecutive losses will appear here.
            </p>
          </div>
        ) : (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        ...
                        streak.status === 'active' ? 'text-amber-500' : 'text-zinc-400'
                      )}>
                    ...
                    <div className="text-xs text-zinc-500">
                      {new Date(streak.start_date).toLocaleDateString()} - {streak.status === 'active' ? 'Now' : new Date(streak.end_date).toLocaleDateString()}
                    </div>
                    <div className="flex items-center gap-3 mt-2">
                      <div>
                        <span className="text-[10px] text-zinc-600 uppercase">Total Loss</span>
                      ...
                      <div>
                        <span className="text-[10px] text-zinc-600 uppercase">Pairs Affected</span>
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-medium text-zinc-300">Session Performance</h3>
        <div className="rounded-lg border border-[#2a2e39] bg-[#1e222d]/50 px-4 py-12 text-center text-xs text-zinc-600">
          ...
          <p className="mt-1 max-w-md mx-auto">
            Daily P&L breakdown showing trading sessions (London, NY, Asian) with performance heatmap.
          </p>
        </div>
      </section>
