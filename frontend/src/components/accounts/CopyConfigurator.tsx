...
export function CopyConfigurator() {
  ...
  const handleRemoveSlave = useCallback(
    (name: string) => {
  ...
  const handleSubmit = useCallback(async () => {
    const name =
      ruleName.trim() || `Copy: ${masterAccount} → ${slaveAccounts.join(', ')}`;
    if (!masterAccount || slaveAccounts.length === 0) {
      addToast({
        title: 'Validation',
        message: 'Select master and at least one slave account',
        severity: 'warning',
        duration: 4000,
      });
      return;
    ...
    try {
      ...
      addToast({
        title: 'Rule created',
        message: `Copy ${masterAccount} → ${slaveAccounts.join(', ')}`,
        severity: 'success',
        duration: 3000,
      });
      setShowForm(false);
      ...
      addToast({
        ...
        message: err instanceof Error ? err.message : 'Check API',
        severity: 'critical',
        duration: 6000,
      });
    }
          ...
          <div className='rounded-lg border border-[#2a2e39] p-4 space-y-3'>
            ...
            <div>
              ...
              <div className='flex gap-2 flex-wrap'>
                ...
                <select
                  id='copy-slave-accounts'
                  onChange={(e) => {
                    const v = e.target.value;
                    if (v && !slaveAccounts.includes(v))
                ...
                >
                  ...
                  {accountNames
                    .filter((n) => !slaveAccounts.includes(n))
                    .map((n) => (
                      <option key={n} value={n}>
                        {n}
                      </option>
            ...
            <div className='flex items-center gap-4'>
              <label className='flex items-center gap-2 text-xs text-[var(--to-text-dim)]'>
                <input
                  ...
                  type='checkbox'
                  checked={scaleByBalance}
                  onChange={(e) => setScaleByBalance(e.target.checked)}
                  className='rounded'
                />
                Scale by balance
              </label>
              <label className='flex items-center gap-2 text-xs text-[var(--to-text-dim)]'>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRiskMultiplier(parseFloat(e.target.value) || 1)
                  }
              ...
              <li
                ...
                className={cn(
                  ...
                  rule.enabled
                    ...
                    : 'border-[#2a2e39]/50 bg-[#1e222d]/30 opacity-70',
                )}
              >
                <div>
