...
export function CapitalAllocator() {
  ...
  const {
    ...
    error,
    refetch,
  } = useAllocationSuggest(totalCapital);
  ...
  const handleExecute = async (accountName: string, allocationUsd: number) => {
    try {
      ...
      addToast({
        ...
        message: `${accountName}: $${allocationUsd.toLocaleString()}`,
        severity: 'success',
        duration: 3000,
      });
    } catch (err) {
      ...
      <CardContent className='space-y-4'>
        ...
        {isLoading ? (
              ...
              {plan.recommendations.map((rec: any) => (
                ...
                >
                  ...
                  <div className='flex justify-between text-[10px] text-[var(--to-text-dim)]'>
                    ...
                    <span>
                      Suggested: $
                      {rec.suggested_allocation_usd.toLocaleString()}
                    </span>
                  </div>
                  <div className='flex items-center justify-between'>
                    ...
                    >
                      {rec.change_usd >= 0 ? '+' : ''}
                      {rec.change_usd.toLocaleString()} (
                      {rec.change_pct >= 0 ? '+' : ''}
                      {rec.change_pct.toFixed(1)}%)
                    </span>
                    <Button
                      size='sm'
                      variant='ghost'
                      ...
                      onClick={() =>
                        handleExecute(
                          rec.account_name,
                          rec.suggested_allocation_usd,
                        )
                      }
                    >
                      {executeAllocation.isPending ? (
          ...
          <p className='text-xs text-amber-600 font-mono py-4 text-center'>
            Failed to load recommendations. Ensure backend API is running and
