...
function useDeleteAlertRule() {
  ...
  return useMutation({
    mutationFn: async (ruleId: number) => {
      ...
      const res = await fetch(`${base}/alerts/rules/${ruleId}`, {
        method: 'DELETE',
      });
    ...
    <div className="tv-card">
      ...
      {showAdd && (
        <div className="px-4 py-3 border-b border-[#2a2e39] space-y-2">
          ...
          >
            {createRule.isPending && <Loader2 className="w-3 h-3 animate-spin" />}
            Create
          </button>
        </div>
      )}
      ...
      <div className="divide-y divide-[#2a2e39]">
        ...
        {rules.map((rule) => (
          <div key={rule.id} className="px-4 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="font-mono text-xs text-zinc-300">{rule.rule_type}</span>
              ...
              <span
                className={cn(
                  ...
                  SEVERITY_COLORS[rule.severity] || 'bg-zinc-500/15 text-zinc-400',
                )}
              >
                {rule.severity}
              </span>
