...
export async function fetchAccountDetail(accountName: string) {
  return apiFetch<any>(`/api/portfolio-control/accounts/${encodeURIComponent(accountName)}`);
}

export async function syncAccount(accountName: string) {
  return apiFetch<any>(`/api/portfolio-control/accounts/${encodeURIComponent(accountName)}/sync`, {
    method: "POST",
  });
}

export async function fetchAllocationSuggest(totalCapital: number, goal: string = 'maximize_sharpe') {
  return apiFetch<any>(`/api/portfolio-control/accounts/allocation-suggest?total_capital=${totalCapital}&goal=${goal}`);
}

export async function executeAllocation(accountName: string, allocationUsd: number) {
  return apiFetch<any>(`/api/portfolio-control/accounts/${encodeURIComponent(accountName)}/allocate`, {
    method: "POST",
...
export async function toggleTradeCopyRule(ruleId: number, enabled: boolean) {
  return apiFetch<any>(`/api/portfolio-control/accounts/trade-copy-rules/${ruleId}/toggle?enabled=${enabled}`, {
    method: "PATCH",
  });
}

export async function fetchTradeCopyLog(limit: number = 50) {
  return apiFetch<any>(`/api/portfolio-control/accounts/trade-copy-log?limit=${limit}`);
}
...
export async function fetchHedgeSuggestions() {
  return apiFetch<HedgeSuggestionApi[]>("/api/portfolio-control/optimizer/hedge-suggestions");
}
...
export async function acceptHedgeSuggestion(suggestionId: number) {
  return apiFetch<any>(`/api/portfolio-control/optimizer/hedge-suggestions/${suggestionId}/accept`, {
    method: "POST",
  });
}

export async function rejectHedgeSuggestion(suggestionId: number) {
  return apiFetch<any>(`/api/portfolio-control/optimizer/hedge-suggestions/${suggestionId}/reject`, {
    method: "POST",
  });
}
...
export async function removeTrailingStop(trailingStopId: number) {
  return apiFetch<any>(`/api/portfolio-control/optimizer/trailing-stops/${trailingStopId}`, {
    method: "DELETE",
  });
}
...
export async function batchPositionAction(payload: {
  ...
  return apiFetch<any>("/api/portfolio-control/optimizer/batch-position-action", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export function getAlertAcknowledgeAllUrl(): string {
  return `${API_BASE_URL}/api/alerts/acknowledge-all`;
}
