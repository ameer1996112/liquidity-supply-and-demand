...
import { Loader2, Plus, Save, CheckCircle2, AlertTriangle, ToggleLeft, ToggleRight } from 'lucide-react';
import { API_BASE_URL, getStrategiesUrl, getStrategyDetailUrl, getStrategyActivateUrl } from '@/lib/api';
  ...
  return (
    <div className="flex gap-4 h-full">
      ...
      <div className="flex-1 flex flex-col tv-card p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="page-title text-sm font-semibold">Strategy Studio</h1>
        ...
        {validationErrors.length > 0 && (
          <div className="bg-red-500/5 border border-red-500/30 rounded p-3 space-y-1">
            {validationErrors.map((e) => (
              <div key={`${e.field}-${e.message}`} className="text-[11px] font-mono text-red-300">
                <span className="font-semibold">{e.field || '(root)'}:</span> {e.message}
              </div>
            ))}
          </div>
        )}
